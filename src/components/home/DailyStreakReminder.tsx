...
const DailyStreakReminder: React.FC<DailyStreakReminderProps> = ({ className = '' }) => {
  ...
  const getStreakColor = () => {
    if (hasDrawnToday) return "from-amber-500/10 to-yellow-500/10 border-amber-500/20";
    if (streak.current >= 7) return "from-orange-500/10 to-red-500/10 border-orange-500/20";
    if (streak.current >= 3) return "from-blue-500/10 to-cyan-500/10 border-blue-500/20";
